        ...
        $LogDir  = "$env:SystemDrive\BlurryFr33thy\logs"
        $LogPath = Join-Path $LogDir "blurry-fr33thy.log"
        if (-not (Test-Path $LogDir)) { New-Item -ItemType Directory -Force -Path $LogDir | Out-Null }
        Add-Content -Path $LogPath -Value ("[" + (Get-Date -Format 'yyyy-MM-dd HH:mm:ss') + "] BlurryFr33thyVerbatim main stage launched. PID=$PID") -ErrorAction SilentlyContinue
        ...
        try {
            ...
            Checkpoint-Computer -Description "BlurryFr33thy - pre-debloat" -RestorePointType "MODIFY_SETTINGS" -ErrorAction SilentlyContinue | Out-Null
...
$StepTwoPs1 = @'
        ...
        # applying the WindowsSettings.reg import (and the rest of the debloat). Without this
        # the RunOnce powershell window just closes and the user sees Settings panes still on.
        $BlurryLogDir  = "$env:SystemDrive\BlurryFr33thy\logs"
        $BlurryLogPath = Join-Path $BlurryLogDir "blurry-fr33thy.log"
        if (-not (Test-Path $BlurryLogDir)) { New-Item -ItemType Directory -Force -Path $BlurryLogDir | Out-Null }
        Add-Content -Path $BlurryLogPath -Value ("[" + (Get-Date -Format 'yyyy-MM-dd HH:mm:ss') + "] StepTwo started. PID=$PID") -ErrorAction SilentlyContinue
        trap {
            $bln = $_.InvocationInfo.ScriptLineNumber
            $bms = "[" + (Get-Date -Format 'HH:mm:ss') + "] StepTwo error at line " + $bln + ": " + $_.Exception.Message
            Add-Content -Path $BlurryLogPath -Value $bms -ErrorAction SilentlyContinue
            Add-Content -Path $BlurryLogPath -Value $_.ScriptStackTrace -ErrorAction SilentlyContinue
            Write-Host ""
            Write-Host "==== BlurryFr33thy StepTwo: caught error at line $bln ====" -ForegroundColor Red
            Write-Host $_.Exception.Message -ForegroundColor Red
            Write-Host "Continuing the rest of StepTwo. Full log: $BlurryLogPath" -ForegroundColor Yellow
            Write-Host ""
            continue
        }
...
try {
    ...
    # wait up to 120s for Discord to finish installing (creates %LOCALAPPDATA%\Discord\app-X.Y.Z\Discord.exe)
    ...
    # Public Desktop safety-net shortcut targeting Update.exe (Discord's official launcher pattern)
...
# Riot Client / Valorant (NA region) -- keep auto-launch so Valorant download starts; safety-net the shortcut after
try {
...
# Logitech G HUB Onboard Memory Manager (request desktop icon explicitly + safety-net shortcut)
try {
...
# BLURRY CHANGE: -NoExit added so the window stays open if StepTwo errors out, instead of vanishing silently.
