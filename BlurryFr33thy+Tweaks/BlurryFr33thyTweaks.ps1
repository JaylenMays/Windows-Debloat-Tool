        ...
        # BLURRY-TWEAKS ADDITION: log path + L() helper used by the TWEAK/CLEAN/EXTRAS menu below.
        $LogPath = "$env:SystemDrive\BlurryFr33thyTweaks\logs\blurry-fr33thy-tweaks.log"
        try { if (-not (Test-Path (Split-Path $LogPath))) { New-Item -ItemType Directory -Force -Path (Split-Path $LogPath) | Out-Null } } catch {}
        ...
        # BLURRY CHANGE: persistent log directory + path
        $LogDir  = "$env:SystemDrive\BlurryFr33thyTweaks\logs"
        $LogPath = Join-Path $LogDir "blurry-fr33thy.log"
        if (-not (Test-Path $LogDir)) { New-Item -ItemType Directory -Force -Path $LogDir | Out-Null }
        Add-Content -Path $LogPath -Value ("[" + (Get-Date -Format 'yyyy-MM-dd HH:mm:ss') + "] BlurryFr33thyTweaks main stage launched. PID=$PID") -ErrorAction SilentlyContinue

        # BLURRY-TWEAKS ADDITION: ask up-front which optional menu items to apply at the end of StepTwo.
        # Saved to disk so the choice survives both reboots; StepTwo reads it back instead of prompting again.
        Clear-Host
        Write-Host ""
        Write-Host "  ================================================================" -ForegroundColor DarkCyan
        Write-Host "  Blurry Fr33thy + Tweaks -- pick now, run unattended" -ForegroundColor Yellow
        Write-Host "  ================================================================" -ForegroundColor DarkCyan
        Write-Host "    1) DEBLOAT  [always]   fr33thy 3-stage debloat + Brave/Steam/Discord/Valorant/Logi" -ForegroundColor Green
        Write-Host "    2) TWEAK    [opt-in]   TweakingGuy AIO + Calypto + BoringBoom NVPI + BOHR V13 power plan" -ForegroundColor Gray
        Write-Host "    3) CLEAN    [opt-in]   Deep refresh -- caches/DriverStore/AppX/EventLog/DISM" -ForegroundColor Gray
        Write-Host "    4) EXTRAS   [opt-in]   Mouse hover + app-kill timeouts (not in any guide)" -ForegroundColor Gray
        Write-Host ""
        Write-Host "  Pick options space-separated (e.g. '2 3 4' for all three)." -ForegroundColor Gray
        Write-Host "  N or empty = DEBLOAT only (no opt-in extras)." -ForegroundColor Gray
        Write-Host "  ================================================================" -ForegroundColor DarkCyan
        $blurryMenuAns  = Read-Host "  Selection"
        $blurryStateDir = "$env:SystemDrive\BlurryFr33thyTweaks"
        if (-not (Test-Path $blurryStateDir)) { New-Item -ItemType Directory -Force -Path $blurryStateDir | Out-Null }
        Set-Content -Path (Join-Path $blurryStateDir "menu-choice.txt") -Value $blurryMenuAns -NoNewline -Force
        Add-Content -Path $LogPath -Value ("[" + (Get-Date -Format 'HH:mm:ss') + "] User picked menu: '" + $blurryMenuAns + "' (saved to menu-choice.txt; will be read at end of StepTwo)") -ErrorAction SilentlyContinue
        Write-Host ""
        Write-Host "  Selection saved. Starting unattended run..." -ForegroundColor Green
        Start-Sleep -Seconds 2

        # BLURRY CHANGE: pre-debloat restore point (in addition to fr33thy's post-debloat one at end of StepTwo)
        try {
            ...
            Checkpoint-Computer -Description "BlurryFr33thyTweaks - pre-debloat" -RestorePointType "MODIFY_SETTINGS" -ErrorAction SilentlyContinue | Out-Null
            Add-Content -Path $LogPath -Value "[$(Get-Date -Format 'HH:mm:ss')] Pre-debloat restore point created." -ErrorAction SilentlyContinue
        } catch {}
...
# download brave (BLURRY CHANGE: Brave instead of Chrome)
Get-FileFromWeb -URL "https://laptop-updates.brave.com/latest/winx64" -File "$env:SystemRoot\Temp\BraveSetup.exe"

# install brave silently
Start-Process -Wait "$env:SystemRoot\Temp\BraveSetup.exe" -ArgumentList "/silent /install"

# install ublock origin lite (Brave honors Chromium ExtensionInstallForcelist)
...
# add brave policies (HW accel off, no background, high efficiency)
...
# remove brave updater services
$services = Get-Service | Where-Object { $_.Name -match 'Brave|brave-update' }
foreach ($service in $services) {
cmd /c "sc stop `"$($service.Name)`" >nul 2>&1"
cmd /c "sc delete `"$($service.Name)`" >nul 2>&1"
}
Get-ScheduledTask -ErrorAction SilentlyContinue | Where-Object { $_.TaskName -match 'Brave' } | Unregister-ScheduledTask -Confirm:$false -ErrorAction SilentlyContinue
...
$StepTwoPs1 = @'
        ...
        # BLURRY DIAGNOSTIC: catch unhandled errors so StepTwo doesn't die silently before
        # applying the WindowsSettings.reg import (and the rest of the debloat).
        trap {
            $bln = $_.InvocationInfo.ScriptLineNumber
            $bms = "[" + (Get-Date -Format 'HH:mm:ss') + "] StepTwo error at line " + $bln + ": " + $_.Exception.Message
            Add-Content -Path $LogPath -Value $bms -ErrorAction SilentlyContinue
            Add-Content -Path $LogPath -Value $_.ScriptStackTrace -ErrorAction SilentlyContinue
            Write-Host ""
            Write-Host "==== BlurryFr33thyTweaks StepTwo: caught error at line $bln ====" -ForegroundColor Red
            Write-Host $_.Exception.Message -ForegroundColor Red
            Write-Host "Continuing the rest of StepTwo. Full log: $LogPath" -ForegroundColor Yellow
            Write-Host ""
            continue
        }
...
# BLURRY CHANGE: keep DesktopAppInstaller (winget) so NVIDIA Control Panel install at ~line 3005 works
$_.Name -notlike '*Microsoft.DesktopAppInstaller*' -and
# BLURRY CHANGE: keep Snipping Tool (ScreenSketch is its package name on Win11)
...
# BLURRY CHANGE: keep Xbox app trio (Game Bar / DVR / overlay neutered separately by reg keys)
$_.Name -notlike '*Microsoft.GamingApp*' -and
$_.Name -notlike '*Microsoft.XboxIdentityProvider*' -and
$_.Name -notlike '*Microsoft.Xbox.TCUI*'
...
# BLURRY CHANGE: auto-download NVIDIA driver via GFW manifest API instead of
# fr33thy's "open browser + manual file pick" flow. RTX 4090 / Win11 64-bit DCH.
# Falls back to F:\ local driver folder, then to file picker if both fail.
        Write-Host "AUTO-DOWNLOADING NVIDIA DRIVER (RTX 4090 / Win11 DCH)`n" -ForegroundColor Yellow
...
try {
    ...
    if ($resp.IDS -and $resp.IDS.Count -gt 0) { $nvUrl = $resp.IDS[0].downloadInfo.DownloadURL }
} catch {}

$InstallFile = "$env:SystemRoot\Temp\nvidia.exe"
$nvOk = $false
if ($nvUrl) {
    try { Get-FileFromWeb -URL $nvUrl -File $InstallFile; $nvOk = $true; Write-Host "Downloaded: $nvUrl" -ForegroundColor Green } catch {}
}
if (-not $nvOk) {
    ...
    if ($localFallback) {
        Copy-Item $localFallback.FullName $InstallFile -Force
        $nvOk = $true
        Write-Host "Using local fallback: $($localFallback.Name)" -ForegroundColor Green
    }
}
if (-not $nvOk) {
    Write-Host "API + local fallback both failed. Falling back to manual file picker..." -ForegroundColor Yellow
...
# download driver
Start-Sleep -Seconds 5
Start-Process "C:\\Program Files\\BraveSoftware\\Brave-Browser\\Application\\brave.exe" "https://www.amd.com/en/support/download/drivers.html"
...
# download driver
Start-Sleep -Seconds 5
Start-Process "C:\\Program Files\\BraveSoftware\\Brave-Browser\\Application\\brave.exe" "https://www.intel.com/content/www/us/en/search.html#sortCriteria=%40lastmodifieddt%20descending&f-operatingsystem_en=Windows%2011%20Family*&f-downloadtype=Drivers&cf-tabfilter=Downloads&cf-downloadsppth=Graphics"
Wait-Process -Name chrome
...
# =============================================================================
# BLURRY CHANGE: silent installs of Steam / Discord / Valorant / Logitech G HUB OMM
# (auto-launch entries deleted from Run keys post-install)
# =============================================================================
        Write-Host "BLURRY APP INSTALLS`n" -ForegroundColor Cyan

# helper: create a Public Desktop .lnk to a known target if the target exists.
# safety net so apps always have a visible shortcut even if their installer skipped it.
function New-BlurryPublicShortcut {
    param([string]$Name, [string]$TargetPath, [string]$WorkingDirectory = "", [string]$Arguments = "")
    if (-not (Test-Path $TargetPath)) { return }
    try {
        $lnk = "$env:Public\Desktop\$Name.lnk"
...
# Steam (BLURRY: keep desktop shortcut, only kill auto-start)
try {
    ...
    New-BlurryPublicShortcut -Name 'Steam' -TargetPath 'C:\Program Files (x86)\Steam\Steam.exe' -WorkingDirectory 'C:\Program Files (x86)\Steam'
} catch {}

# Discord (let installer finish + place shortcut, then close it; keep desktop shortcut)
try {
    Get-FileFromWeb -URL 'https://discord.com/api/download?platform=win' -File "$env:SystemRoot\Temp\DiscordSetup.exe"
    Start-Process "$env:SystemRoot\Temp\DiscordSetup.exe"
    $deadline = (Get-Date).AddSeconds(120)
    while ((Get-Date) -lt $deadline) {
        $appDir = Get-ChildItem "$env:LOCALAPPDATA\Discord\app-*" -Directory -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
        if ($appDir -and (Test-Path (Join-Path $appDir.FullName 'Discord.exe'))) { Start-Sleep -Seconds 8; break }
        Start-Sleep -Seconds 3
    }
    Get-Process Discord, DiscordSetup, Update -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue
    ...
    if (Test-Path "$env:LOCALAPPDATA\Discord\Update.exe") {
        New-BlurryPublicShortcut -Name 'Discord' -TargetPath "$env:LOCALAPPDATA\Discord\Update.exe" -WorkingDirectory "$env:LOCALAPPDATA\Discord" -Arguments '--processStart Discord.exe'
    }
} catch {}

# Riot Client / Valorant (NA region)
try {
    ...
    New-BlurryPublicShortcut -Name 'Riot Client' -TargetPath 'C:\Riot Games\Riot Client\RiotClientServices.exe' -WorkingDirectory 'C:\Riot Games\Riot Client' -Arguments '--launch-product=valorant --launch-patchline=live'
} catch {}
...
try {
    ...
    Start-Process -Wait "$env:SystemRoot\Temp\LogiOMM.exe" -ArgumentList '/SILENT /SUPPRESSMSGBOXES /NORESTART /TASKS="desktopicon"'
    New-BlurryPublicShortcut -Name 'Onboard Memory Manager' -TargetPath 'C:\Program Files\Logitech\OnboardMemoryManager\OnboardMemoryManager.exe' -WorkingDirectory 'C:\Program Files\Logitech\OnboardMemoryManager'
} catch {}

# =============================================================================
# BLURRY-TWEAKS ADDITION: 4-option menu (DEBLOAT / TWEAK / CLEAN / EXTRAS)
# Menu code copied verbatim from Blurry4.ps1 lines 996-1717.
# =============================================================================
...
L "Final-stage menu (reading user choice from main-stage prompt)" 'STAGE'
# BLURRY-TWEAKS: read the menu choice that was made at the start of the main stage
# (saved to C:\BlurryFr33thyTweaks\menu-choice.txt). No second prompt -- script is unattended.
$menuChoiceFile = "$env:SystemDrive\BlurryFr33thyTweaks\menu-choice.txt"
$menuAns = ''
if (Test-Path $menuChoiceFile) {
    try { $menuAns = (Get-Content $menuChoiceFile -Raw -ErrorAction Stop).Trim() } catch { $menuAns = '' }
}
Write-Host ""
Write-Host "  ================================================================" -ForegroundColor DarkCyan
Write-Host "  Blurry Fr33thy + Tweaks -- applying main-stage selection" -ForegroundColor Yellow
Write-Host "  ================================================================" -ForegroundColor DarkCyan
Write-Host "    1) DEBLOAT  [done]    fr33thy 3-stage debloat + Brave/Steam/Discord/Valorant/Logi" -ForegroundColor Green
Write-Host "    2) TWEAK    [opt-in]  TweakingGuy AIO + Calypto + BoringBoom NVPI" -ForegroundColor Gray
Write-Host "    3) CLEAN    [opt-in]  Deep refresh -- caches/DriverStore/AppX/EventLog/DISM" -ForegroundColor Gray
Write-Host "    4) EXTRAS   [opt-in]  Mouse hover + app-kill timeouts" -ForegroundColor Gray
Write-Host ""
Write-Host "    Selection (from main-stage prompt): '$menuAns'" -ForegroundColor Cyan
Write-Host "  ================================================================" -ForegroundColor DarkCyan
...
if ($doTweak) {
    ...
    # BLURRY-TWEAKS REMOVAL: BOHR V13 power plan removed per user request (was layer D of TWEAK).
    # Ultimate Performance power plan set earlier in TWEAK stays active.
...
# end of TWEAK / CLEAN / EXTRAS block
...
# BLURRY CHANGE: -NoExit added so the window stays open if StepTwo errors out.
cmd /c "reg add `"HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\RunOnce`" /v `"StepTwo`" /t REG_SZ /d `"powershell.exe -NoExit -nop -ep bypass -WindowStyle Maximized -f $env:SystemRoot\Temp\StepTwo.ps1`" /f >nul 2>&1"
